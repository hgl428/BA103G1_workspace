...
public class PetViewSearchServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		if ("petView".equals(action)) {
			...
			try {
				...
				if(list.size() == 0){     //如果標題是空白的話，就說標題請勿空白
                	errorMsgs.add("查無此條件的動物");
                }
				...
				if (!errorMsgs.isEmpty()) {
					...
					RequestDispatcher failureView = req
							.getRequestDispatcher("/JSP/IpetB/adoption.jsp");
					failureView.forward(req, res);
					return;//程式中斷
				}
				/***************************
				 * 3.成功並轉交(Send the Success view)
				 ************/
